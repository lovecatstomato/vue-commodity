  ...
  <div>
    ...
    <el-form :inline="true" :model="ReviseList" label-width="140px">
      ...
      <el-form-item>
        ...
        <el-table :data="ReviseList.poitems" border style="width: 1000px">
...
export default {
  ...
  methods: {
    ...
    //明细删除按钮
    delet(index) {
      this.ReviseList.poitems.splice(index, 1);
    },
    //选择商品
    ...
    selectProduct(currentRow) {
      ...
      //currentRow参数是选中行的值
      //获得选中明细的数据
      let item = this.ReviseList.poitems[this.selectProductIndex];
    ...
    //采购单列表数据封装请求事件
    Purchase() {
      this.ReviseList = {...this.$route.query.data, poitems: []}; //获取传参过来的数值
    },
    //明细
    Su() {
      let poid = this.ReviseList.poId;
      this.$axios
        .get("/main/purchase/pomain/queryItem", {
          params: {
            poId: poid,
          },
        })
        .then((res) => {
          ...
          console.log(res);
          this.ReviseList.poitems = res;
        });
    },
    tain() {
      // 确定关闭
      this.$axios
        .post("/main/purchase/pomain/update", JSON.stringify(this.ReviseList), {
          headers: {
